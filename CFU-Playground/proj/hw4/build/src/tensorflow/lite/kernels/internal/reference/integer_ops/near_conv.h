...
inline void ConvPerChannel(
    ...
    int max_im2col_index = 0;
    int patch_size = filter_height * filter_width * filter_input_depth;

    for (int batch = 0; batch < batches; ++batch) {
        // Clearing or reinitializing im2col arrays for each batch
        memset(input_im2col, 0, sizeof(input_im2col));
        memset(weight_im2col, 0, sizeof(weight_im2col));
        for (int out_y = 0; out_y < output_height; ++out_y) {
            ...
            for (int out_x = 0; out_x < output_width; ++out_x) {
                ...
                for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
                    ...
                    // Initialize the index for flattening the patch into im2col
                    int im2col_patch_start_index = (out_y * output_width + out_x) * patch_size;
                    for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                        const int in_y = in_y_origin + dilation_height_factor * filter_y;
                        for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                            const int in_x = in_x_origin + dilation_width_factor * filter_x;

                            // Loop over each input channel within the filter
                            for (int in_channel = 0; in_channel < filter_input_depth; ++in_channel) {
                                int im2col_index = im2col_patch_start_index + (filter_y * filter_width + filter_x) * filter_input_depth + in_channel;
                                max_im2col_index = std::max(max_im2col_index, im2col_index);
                                // Zero padding check
                                if (in_x < 0 || in_x >= input_width || in_y < 0 || in_y >= input_height) {
                                    input_im2col[im2col_index] = input_offset;  // Set zero-padding value
                                    weight_im2col[im2col_index] = 0;
                                } else {
                                    ...
                                    input_im2col[im2col_index] = input_val + input_offset;
                                    weight_im2col[im2col_index] = filter_val;
                                }
                            }
                        }
                    }
        ...
        for (int out_y = 0; out_y < output_height; ++out_y) {
            for (int out_x = 0; out_x < output_width; ++out_x) {
                for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
                    int32_t acc = 0;  // Accumulator for the convolution result

                    // Compute the starting index for this patch in im2col
                    int patch_index = (out_y * output_width + out_x) * patch_size;

                    for (int kernel_index = 0; kernel_index < patch_size; ++kernel_index) {
                        // Multiply and accumulate
                        acc += input_im2col[patch_index + kernel_index] * weight_im2col[patch_index + kernel_index];
                    }

                    if (bias_data) {
                        acc += bias_data[out_channel];  // Add bias if present
                    }

                    // Apply quantization parameters: multiplier and shift
                    acc = MultiplyByQuantizedMultiplier(
                        acc, output_multiplier[out_channel], output_shift[out_channel]);
                    acc += output_offset;                        // Add output offset
                    acc = std::max(acc, output_activation_min);  // Apply activation min limit
                    acc = std::min(acc, output_activation_max);  // Apply activation max limit
