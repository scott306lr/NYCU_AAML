...
inline void ConvPerChannel(
    ...
    */
    for (int batch = 0; batch < batches; ++batch) {
        // im2col for input
        int col_height = filter_height * filter_width * filter_input_depth;
                ...
                for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
                    ...
                    for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                        ...
                        for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                            ...
                            for (int in_channel = 0; in_channel < filter_input_depth; ++in_channel) {
                                int col_index = (filter_y * filter_width + filter_x) * filter_input_depth + in_channel;
                                int input_index = (out_y * output_width + out_x) * col_height + col_index;

                                if (in_x >= 0 && in_x < input_width && in_y >= 0 && in_y < input_height) {
                                    input_im2col[input_index] = input_data[Offset(input_shape, batch, in_y, in_x, in_channel + group * filter_input_depth)] + input_offset;
                                } else {
                                    // Zero padding
                                    input_im2col[input_index] = input_offset;
                                }
                            }
                        }
                    }
