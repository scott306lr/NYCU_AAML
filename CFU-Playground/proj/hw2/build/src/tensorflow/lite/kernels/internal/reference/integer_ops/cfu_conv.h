                    ...
                    for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                        ...
                        for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                            ...
                            for (int in_channel = 0; in_channel < input_depth; in_channel += 4) {
                                int32_t input_val = 0, filter_val = 0;

                                int depth_limit = std::min(4, input_depth - in_channel);
                                if (depth_limit == 4) {  // read 4 bytes at a time
                                    input_val = *reinterpret_cast<const int32_t*>(input_ptr);
                                    filter_val = *reinterpret_cast<const int32_t*>(filter_ptr);
                                } else {  // partial outside 4-byte boundary, read byte by byte
                                    for (int d = 0; d < depth_limit; d++) {
                                        input_val = (input_val << 8) | (static_cast<int32_t>(input_ptr[d]) & 0xFF);
                                        filter_val = (filter_val << 8) | (static_cast<int32_t>(filter_ptr[d]) & 0xFF);
                                    }
                                }
